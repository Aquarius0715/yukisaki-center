...
| POST | `/v1/routes` | 確定した出発地・目的地からAIおすすめ・走りやすさ優先・距離優先の最大3経路を取得 |
...
Webは`mode=comparison`を指定する。`balanced`は「AIおすすめ」、`most_drivable`は「走りやすさ優先」、`distance_priority`は「距離優先」と表示する。「AIおすすめ」の経路は総合バランスの決定的ルールで確定済みであり、AIはその選択理由と他候補との差だけを説明する。

`POST /v1/routes`の`origin`には、目的地選択時にブラウザのGeolocation APIから取得した現在地を使用する。地図の現在地マーカーも同じ座標を共有する。位置情報が未許可・取得不能・タイムアウトの場合は固定デモ座標へフォールバックせず、利用者へ再設定・再試行を案内する。現在地がデモ対象の長岡市域外の場合も、その旨を表示して経路APIは呼び出さない。
