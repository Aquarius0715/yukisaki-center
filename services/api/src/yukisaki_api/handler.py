...
def handle(event: dict[str, Any], service: MapService) -> dict[str, Any]:
    ...
    if path == "/healthz":
        return _response(200, {"status": "ok"}, "public,max-age=5,s-maxage=5")
    try:
        query = MapQuery.parse(event.get("queryStringParameters"))
        if path == "/v1/road-segments":
            return _response(200, service.roads(query), MAP_CACHE_CONTROL)
        if path.startswith("/v1/road-segments/"):
            ...
            return (
                _response(200, feature, DETAIL_CACHE_CONTROL)
                if feature
                else _response(404, {"error": {"code": "not_found", "message": "road segment was not found"}})
            )
        if path == "/v1/snowplows":
            return _response(200, service.snowplows())
        if path == "/v1/map/snapshot":
            return _response(200, service.snapshot(query), MAP_CACHE_CONTROL)
