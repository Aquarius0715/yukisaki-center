...
def score_message(message: dict[str, Any]) -> dict[str, Any]:
    ...
    with connect_database() as connection:
        with connection.cursor() as cursor:
            ...
            """)
            cursor.execute("SELECT 1 FROM data_load_runs WHERE run_id = %s", (message["processingRunId"],))
            if cursor.fetchone() is None:
                raise RuntimeError("GPS PostgreSQL load is not complete; retry scoring later")
            results = [score_segment(item) for item in _input_rows(cursor, segment_ids, message["latestObservedAt"])]
            if len(results) != len(segment_ids):
                raise ValueError("one or more scoring road segments do not exist")
            score_run_id = f"score-{message['processingRunId']}"
            body = b"".join(
                (json.dumps(item, ensure_ascii=False, sort_keys=True, default=str) + "\n").encode()
                for item in results
            )
            checksum = hashlib.sha256(body).hexdigest()
            snapshot_date = message["latestObservedAt"][:10]
                ...
                cursor.execute(
                    ...
                    (
                        ...
                        result["confidence"], json.dumps({
                            ...
                            "source_run_ids": [message["processingRunId"]],
