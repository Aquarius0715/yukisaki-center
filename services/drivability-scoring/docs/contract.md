...
入力は`segment_id`、`data_timestamp`、降雪量、気温、勾配、道路種別（`road_type`、狭い生活道路の判定に使用）、最終除雪時刻、消雪パイプ状態を含む。出力は0〜100の`score`、0〜1の`confidence`、加点・減点根拠、入力値、`rule_version`、`is_simulated`を含む。`road_type`が`residential`・`living_street`・`unclassified`の区間は`narrow_road`で-10点、`service`・`track`の区間は-15点とする。`road_type`は`confidence`の算出には含めない。
