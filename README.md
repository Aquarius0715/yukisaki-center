...
すべてのサービスにDockerfileを配置している。現在の収集基盤はAWS CDKでLambda、ECS Fargate、EventBridge、S3、SQS、CloudWatchを管理する。除雪車GPSモックは3台を1つのFargateタスクで動かし、EventBridgeとSQS経由でS3、PostgreSQL、走りやすさ指数へ反映する。
