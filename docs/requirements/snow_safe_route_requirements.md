...
| データ取得・地図対象地域 | 新潟県長岡市の行政区域全体を対象とする |
| デモ対象 | 2026年1月23日の長岡市全域を対象とし、石動南町を初期確認地点とする |
...
- 経路検索時のおすすめ出発時刻は、除雪車の通過間隔から学習した統計モデル（ロジスティック回帰）で予測してよい。ただし、この予測は走りやすさ指数・経路コスト・順位を変更せず、レスポンス上も予測値であることを明示する（[経路探索要件](#10-経路探索要件)参照）。
- 指数計算に使用する実データは、利用条件を確認できる公開データに限定する。
- 消雪パイプと除雪車GPSは、本MVPでは仮データを使用する。
- Apple Maps（MapKit JS）は背景地図と表示に利用し、独自の走りやすさを考慮した経路探索はAWS側で実行する。
...
- AI抽出結果と地点候補の確認・修正
- 「AIおすすめ」「走りやすさ優先」「距離優先」の3経路比較
- 候補経路間の違いを説明するAI
- 除雪車通過間隔の統計モデルによる、おすすめ出発時刻の予測表示
- 推奨経路上の危険ポイント表示
...
- ターンバイターン方式の車載ナビ
- ユーザー投稿・口コミ機能
- 走りやすさを予測する機械学習モデル（走りやすさ指数そのものの機械学習化。おすすめ出発時刻のための除雪車通過タイミング予測は対象内であり、指数計算とは別系統として扱う）
- 安全性または通行可能性の保証

### 5.3 デモ対象と再現性

- デモ日は**2026年1月23日**、データ取得範囲は**新潟県長岡市全域**とする。石動南町は初期確認地点として維持する。
- デモ実行時は、対象日時の気象・道路・仮データを固定したスナップショットまたはfixtureを使用する。
- 現在時刻の定期収集データは運用確認に利用できるが、デモ結果の再現性を損なうため、デモ画面・デモ用指数計算の入力には使用しない。
- デモ用データには対象日時、対象地域、出典、取得・作成時刻、`is_simulated`を記録する。
...
|---|---|---|---|---|
| 気象観測 | 気温、湿度、降水量、降雪量、積雪深、天気コード、風速・突風 | Open-Meteo Historical Weather API | デモ基準時刻の3時間前〜基準時刻を手動取得 | 積雪・凍結・吹雪リスク算出 |
| 気象予報 | 予測気温、湿度、降雪・降水、積雪深、天気コード、風速・突風 | Open-Meteo Historical Forecast API | デモ基準時刻の1〜3時間後の過去予報を手動取得 | 近未来の悪化傾向の反映 |
...
![走りやすさ指数・経路探索・AI説明の処理フロー](../architecture/drivability_processing_flow.png)
...
- 経路検索の結果に、おすすめ出発時刻を予測情報として付加できる。
- 対象は代表経路（`comparison`モードでは総合バランス経路、それ以外は先頭経路）のうち、直近60分以内に除雪されていない区間とする。
- 予測は、除雪車GPS仮データの区間別通過履歴（間隔）を学習したロジスティック回帰で、候補となる待ち時間（0〜90分の範囲で複数）ごとに「その時刻までに対象区間が除雪済みとなっている確率」を推定する。
- 推定確率があらかじめ定めた閾値を満たす最短の待ち時間を、おすすめ出発時刻として返す。対象区間が無い場合は即時出発を、学習に十分な通過履歴が無い場合は情報不足を返す。
- この予測は走りやすさ指数・経路コスト・経路順位を変更しない。指数はルールベースの算出結果のまま維持する。
- レスポンスでは、予測値であること（`is_prediction`）と仮データに基づくこと（`is_simulated`）を明示する。

## 11. 機能要件

| ID | 機能 | 要件 |
|---|---|---|
| FR-01 | 地図表示 | Apple Maps（MapKit JS）を背景として対象地域を表示できること |
...
| FR-09 | 経路比較 | AIおすすめ、走りやすさ優先、距離優先の最大3経路を比較できること |
...
| FR-18 | おすすめ出発時刻 | 経路検索結果に、除雪車通過タイミングの統計予測に基づくおすすめ出発時刻を、予測・仮データである旨とあわせて表示できること |

## 12. 画面要件

### 12.1 メイン地図画面

- Apple Maps（MapKit JS）を表示する。
...
- 危険区間数
- 主な危険要因
- おすすめ出発時刻（除雪車通過タイミングの統計予測。予測値であることを明示）
- データ基準時刻
- 実データ・仮データの区分
- 他の経路候補との比較
...
![利用者入力から経路案内までのフローチャート](../architecture/user_ai_route_flow.png)
...
![雪道安全経路案内システムのシステム構成図](../architecture/system_architecture.png)
...
| POST | `/v1/routes` | 出発地、目的地、モードから経路を探索。レスポンスにおすすめ出発時刻の予測（`departure_recommendation`）を含む |
...
- 通信はHTTPSとする。
- MapKit JSトークンはApple Developer側で利用ドメインを制限する。
...
- Apple MapsおよびMapKit JSの利用規約に従う。
...
| AC-22 | おすすめ出発時刻が予測値・仮データである旨とあわせて表示され、走りやすさ指数・経路順位を変更しないこと |
...
- Apple Mapsの標準経路と本システムの独自経路は一致しない場合がある。
- LLMの文章は補足説明であり、指数の正当性を保証するものではない。
- おすすめ出発時刻は、仮の除雪車GPSが生成した通過パターンから学習した統計的な予測であり、実際の除雪計画や到着を保証するものではない。
