...
新潟県長岡市石動南町の道路ネットワークに対し、所要時間だけでなく、道路区間ごとの走りやすさ指数、信頼度、勾配、消雪設備、除雪履歴、通行規制を考慮した候補経路を最大3件返す。

経路の数値、通行対象外区間、順位、危険区間は決定的なルールとグラフ探索で確定する。LLMは確定済み候補の説明だけを担当し、探索結果を変更しない。

本書は設計を定義するものであり、現時点ではPostGIS/pgRouting道路グラフ、公開経路API、AWS実行基盤は未実装である。
...
Google Routes APIの`routeModifiers`は、高速道路、有料道路、フェリー等を避ける用途であり、本プロジェクトの区間別指数を任意コストとして渡す用途には適合しない。GraphHopperのCustom ModelやValhallaのDynamic Costingは柔軟だが、RDSにある4,944区間の独自ID・最新指数と外部グラフの対応を別途維持する必要がある。
...
現在の`road_segments`は地図表示と区間別属性の格納には使えるが、経路探索グラフとしては未完成である。

| 項目 | 現在 | 経路探索に必要な対応 |
|---|---|---|
| `segment_id` | あり | 維持する |
| LineString | JSONB GeoJSON | PostGIS `geometry(LineString, 4326)`へ追加ロードする |
| 長さ | あり | 維持する |
| 始点・終点座標 | S3 GeoJSONにはあり | DBノードへ正規化する |
| OSM `u/v` | ID生成時には利用するが出力区間へ未保持 | 収集・curated契約に保持する |
| 一方通行 | S3 GeoJSONにはあり | 方向付きedgeの`cost/reverse_cost`へ反映する |
| 制限速度 | S3 GeoJSONにはあり | 正規化し、欠損時は道路種別の既定速度を使用する |
| access/service | S3 GeoJSONにはあり | 自動車通行対象外を除外する |
...
routing_nodes (
  ...
  is_osm_intersection boolean not null,
...
1. 選択モードでpgRouting `pgr_dijkstra`により最良経路を取得する。
2. `pgr_KSP`で選択モードの上位候補を最大10件生成する。
3. 時間優先・バランス・走りやすさ優先の各最良経路も候補プールへ加える。
4. edge列が同じ経路を削除する。
5. edge集合の重複率が80%以上の候補を類似経路としてまとめる。
6. `max_detour_minutes`、通行止め、ループ、対象地域外を検証する。
7. 利用者が選択したモードのコストで再順位付けし、最大3件返す。
...
新しい`YukisakiRoutePlanning-dev`スタックを作る設計とする。
...
4,944区間の対象地域ではpgRoutingで十分な見込みだが、実測で判断する。インデックス、SQL、候補Kを先に調整し、根拠なく大型インスタンスへ変更しない。
