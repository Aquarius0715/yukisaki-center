...
2. 出発地・目的地を囲む約3km余白の回廊内で`pgr_KSP`の上位3件を生成する。接続できない場合だけ約8km余白へ広げて再探索する。
3. 最速候補から`max_detour_minutes`を超える候補を除外する。
4. edge集合の重複率が95%以上の候補を類似経路として優先候補から除外する。
5. 優先候補が3件未満の場合は、許容迂回時間内にある同一でないK最短候補から補充する。
6. 利用者が選択したモードのコストで順位を確定し、最大3件返す。
