...
入力緯度経度からPostGISで最寄りの`routing_edges`を検索し、そのedgeのsource/targetのうち入力に近い`routing_nodes`を探索始終点として選ぶ。

1. WGS84緯経度と対象地域内かを検証する。
2. 近傍の利用可能なedgeをGiST indexで検索する。
3. edgeの方向・accessを考慮して候補ノードを選ぶ。
4. 最大スナップ距離を500mとし、超える場合は400エラーにする。地点検索が大規模施設や敷地の中心座標を返す場合も、道路入口相当へスナップできる範囲とする。
5. 応答にスナップ後座標、node ID、選択ノードまでの距離`distance_m`、最寄り道路までの距離`road_distance_m`を含める。

最大スナップ距離は最寄りノードではなく最寄りedgeまでの距離で判定する。これにより、長い道路区間の途中にある地点を道路から離れていると誤判定しない。MVPの経路自体は選択したedge端点から開始し、将来、edge途中までの正確な部分コストが必要になった場合は、pgRoutingのwithPoints系関数またはリクエスト内の仮想ノードを使用する。
...
| `distance_priority` | 0.2 | 0.2 | 距離を固定速度換算した基礎コストで、短い走行距離を優先 |
...
1. グラフ版、指数版、基準時刻、モード、allow-list済み条件から決定的なキャッシュキーを作り、該当する動的コスト表がなければRDSへ一度だけ生成する。
2. 出発地・目的地を囲む約3km余白の回廊内で`pgr_KSP`の上位8件を生成する。接続できない場合だけ約8km余白へ広げて再探索する。
3. 最速候補から`max_detour_minutes`を超える候補を除外する。
4. 単一モードではedge集合の重複率が95%以上の候補を類似経路として優先候補から除外する。
5. `comparison`モードでは、総合バランス、走りやすさ、距離の3プロファイルを同じ入力版で探索する。総合バランス候補を基準に、全体の道路区間重複率75%以下かつ始終点各10%を除く中間区間60%以下を優先する。該当しない場合だけ全体85%・中間75%まで緩和する。
6. 緩和上限を超える類似経路で3件へ水増しせず、条件を満たす重複しない候補だけを最大3件返す。
...
- `balanced`: 総合バランスコストが最小。Webでは「AIおすすめ」と表示し、AIは確定済みの推奨理由だけを説明する
...
連続する危険区間はGeometryが連続し、同じ主要因を持つ場合に1つの`hazard_group`へ集約する。AIへ渡すのは集約済みの根拠である。各`hazard_group`は判定ルール名(`factors`)に加え、道路名・区間長・最大勾配・1時間降雪量・気温・最終除雪時刻のうち根拠がある値だけを`evidence`として保持する。同一の`factors`を持つ危険区間でもAI説明が画一化しないよう、抽象的なルール名だけでなく具体的な数値をAIへ渡す。
...
- Apple Maps Search / Autocomplete: 地名・施設名・住所から緯経度候補を取得
- Apple MapKit JS: 背景地図と独自GeoJSON経路を表示

### 使用しない範囲

- Apple Maps Directionsを経路生成に使用しない
- 外部Routes APIの順位を本システムの推奨順位として利用しない
- 外部経路を独自道路へ正確にマッチできない状態で指数を付与しない
- 外部API失敗時に推測した地点・経路を返さない

交通情報は現時点の経路レスポンスに含まれない。LLMは交通量、渋滞、事故、規制を推測せず、根拠が追加されるまで「交通情報を評価した」と表現しない。
