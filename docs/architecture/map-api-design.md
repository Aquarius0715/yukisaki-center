...
Webフロントエンドへ、道路形状、走りやすさ指数、消雪パイプ、最終除雪情報、除雪車30台の最新位置をHTTPSのJSON / GeoJSON APIとして提供する。
...
現在の30台・5秒間隔ではRESTポーリングで十分である。1秒未満の更新、車両数・同時閲覧者の大幅な増加が必要になった場合に限り、除雪車差分配信をWebSocketへ追加する。道路GeoJSONは将来もRESTまたはベクトルタイルで配信する。
...
| スモークテスト | `/healthz`、道路、除雪車30台、一括取得、CORS |
