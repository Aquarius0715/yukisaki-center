...
Amazon API Gateway HTTP API
    |
    | Lambda proxy integration v2
    v
Map API Lambda（Docker image / ARM64）
    |           |
    |           +--> AWS Secrets Manager
    |                 共通DB認証情報
    |
...
| パラメータ | 必須 | 形式 | 既定値・制限 |
|---|---|---|---|
| `bbox` | いいえ | `west,south,east,north` | `138.74,37.40,138.84,37.49` |
...
`road_segments_enriched`を基点に、各道路区間の最新情報を結合する。

| 公開プロパティ | PostgreSQL取得元 |
|---|---|
| `segment_id`、道路形状・名称・種別・勾配 | `road_segments_enriched` |
| `snow_pipe`、稼働状態、効果 | `road_segments_enriched` |
...
現在はPostGISでbbox検索せず、4,944道路をPostgreSQLから読み、Lambda内でGeometryの外接矩形とリクエストbboxの交差を判定している。デモ規模では全道路応答約3.26 MB、実測約1秒である。道路数または同時アクセスが増えた場合は、PostGISの空間インデックスによるDB側絞り込み、ページング、MVT配信へ移行する。
