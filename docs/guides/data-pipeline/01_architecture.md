...
気象と消雪パイプではprivate RDSを使用するため、起動中は料金が発生する。デモ環境を長期間使わない場合は2つのRDSを停止し、必要なS3正本を保持する。Weather、道路、消雪パイプmanifestのEventBridge Ruleは既定で無効にし、開発・デモ時だけ`env:start`で有効化する。
