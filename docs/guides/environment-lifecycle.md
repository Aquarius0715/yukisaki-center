...
| `runtime` | 開発・デモ時だけ起動 | 気象・消雪パイプRDS、DBへ接続する処理 |
...
2. Weather Collector、Weather Loader、Snow Pipe DB Loaderの予約同時実行数を0にする。
3. 実行中の道路Fargateタスクへ停止を要求する。
4. 気象RDSと消雪パイプRDSへ停止を要求する。
5. S3、ECR、Secrets Manager、SQS、DLQ、ログは維持する。

`env:start`は2つのRDSを並行して起動要求し、両方が利用可能になるまで待つ。その後に関連Lambdaと3つのEventBridge Ruleを有効化する。RDSの起動には数分以上かかる場合があるため、デモ直前ではなく余裕を持って実行する。
