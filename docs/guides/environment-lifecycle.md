...
| `runtime` | 開発・デモ時だけ起動 | 共通RDS、DBへ接続する処理 |
...
4. 気象・道路・消雪パイプ共通RDSへ停止を要求する。
5. S3、ECR、Secrets Manager、SQS、DLQ、ログは維持する。

`env:start`は共通RDSを起動して利用可能になるまで待つ。その後に関連Lambdaと3つのEventBridge Ruleを有効化する。RDSの起動には数分以上かかる場合があるため、デモ直前ではなく余裕を持って実行する。
