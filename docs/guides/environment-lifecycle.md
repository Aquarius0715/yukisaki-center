...
| 種類 | 扱い | 現在の例 |
|---|---|---|
| `persistent` | 開発停止中も維持 | S3原本、Kinesis、SQS、DLQ、ECR、ログ |
...
6. Kinesis、S3、ECR、Secrets Manager、SQS、DLQ、ログは維持する。
...
- Kinesis Streamはリアルタイム入口として維持するため、`env:stop`後も1 shard分の時間料金が残る。
