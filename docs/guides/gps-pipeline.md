...
```sql
SELECT vehicle_id, observed_at, received_at, latitude, longitude,
...
- 旧石動南町データ4,944区間では3台それぞれ約80.47kmだった。長岡市全域の再収集後は距離と周回時間を再計測し、経路の和集合が新しい全区間を含むことをテストで保証する。
- `observed_at`は2026-01-23のデモシナリオ時刻、`received_at`は実受信時刻である。最新位置とWeb更新の判定には`received_at`由来の`data_timestamp`を使う。
